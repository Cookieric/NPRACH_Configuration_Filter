...
TEST_LOG=eNB_MSG2_Test_001.log
HOME_PATH=`pwd`
PHY_PATH=../exec
AS_PATH=../exec
CTRL_PATH=../exec
...
MAC_COMMAND="--rachperiod ${1} --rachstart ${6} --rachrepeat ${2} --rachscofst 0 --rachnumsc 2 --rmax ${3} --gvalue ${4} --candidate ${7} --dlcqi 7 --rawindow ${5} --rartbsindex 3 --rarsfindex 1 --dlrepeat 0 --s2tbs 2" #case1
...
cd Result
mv ShellCopy.sh run_MSG2_Test${8}.sh
cat run_MSG2_Test${8}.sh |  sed "12c MAC_COMMAND=\"$MAC_COMMAND\"" > log.sh
# rm run_MSG1_Test${8}.sh
mv log.sh run_MSG2_Test${8}.sh
mkdir -p MAC_COMMAND_Result
cd MAC_COMMAND_Result
cat ../run_MSG2_Test${8}.sh | sed '12q;d' > MAC_COMMAND_${8}.txt
exit 0;
